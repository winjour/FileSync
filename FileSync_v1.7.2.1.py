...
class FileSyncApp:
    def __init__(self, root):
        # 现有的初始化代码...
        self.root = root
        self.root.title("FileSync_v1.7.2.1 -- 文件同步备份工具")
        ...
        self.max_workers = min(self.cpu_count * 2, 16)  # 最多16个线程
        self.hash_lock = Lock()  # 用于哈希计算状态更新的锁
        self.sync_stats = {"processed": 0, "total": 0}  # 同步统计
    ...
    def get_folder_state(self, folder_path, progress_callback=None):
        """获取文件夹中所有文件的状态（路径和哈希值）- 并行优化版本"""
        states = {}
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交所有任务
            future_to_path = {
                executor.submit(self._get_file_hash_safe, file_path): (file_path, rel_path)
            ...
            for future in as_completed(future_to_path):
                file_path, rel_path = future_to_path[future]
                try:
                    ...
                    if progress_callback:
                        ...
                        self.root.after(0, lambda p=progress: progress_callback(p))
    ...
    def _get_file_hash_safe(self, file_path):
        """安全的文件哈希计算方法，处理异常"""
        try:
    ...
    def update_sync_progress(self, progress):
        ...
        if hasattr(self, 'status_var'):
            if progress < 100:
                self.status_var.set(f"正在分析文件... {progress}%")
            else:
                self.status_var.set("分析完成，开始同步...")
    ...
    def _sync_single_file_task(self, task):
        """执行单个文件同步任务"""
        action, source_path, dest_path, rel_path = task
        ...
        try:
            ...
            result['size_str'] = self.get_file_size(source_path)
            result['bytes'] = file_size_bytes
    ...
    def perform_contribute_sync(self, source_dir, dest_dir, silent=False):
        """执行贡献模式同步（原有模式）- 并行优化版本"""
        # 获取源文件夹和目标文件夹的状态
        self.status_var.set("正在分析源文件夹...")
        src_states = self.get_folder_state(source_dir, self.update_sync_progress)

        self.status_var.set("正在分析目标文件夹...")
        dest_states = self.get_folder_state(dest_dir, self.update_sync_progress)
        ...
        for rel_path, src_hash in src_states.items():
            ...
            if rel_path not in dest_states:
                # 新增文件
                sync_tasks.append(('add', source_path, dest_path, rel_path))
            elif dest_states[rel_path] != src_hash:
                # 更新文件
                sync_tasks.append(('update', source_path, dest_path, rel_path))

        if not sync_tasks:
            self.status_var.set("没有需要同步的文件")
            return stats

        # 使用线程池并行处理文件同步
        self.status_var.set(f"开始同步 {len(sync_tasks)} 个文件...")

        with ThreadPoolExecutor(max_workers=min(4, self.max_workers)) as executor:  # 限制同时写入的线程数
            future_to_task = {
                executor.submit(self._sync_single_file_task, task): task
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    self.root.after(0, lambda: self.log_tree.insert("", 0, values=(
                        timestamp, action_text, rel_path, result['size_str'], status_text)))
    ...
    def perform_oneway_sync(self, source_dir, dest_dir, silent=False):
        ...
        # 收集需要删除的文件
        for rel_path in dest_states.keys():
            if rel_path not in src_states:
                dest_path = os.path.join(dest_dir, rel_path)
                delete_tasks.append(('delete', dest_path, rel_path))
        ...
        # 然后处理文件删除
        if delete_tasks:
            self.status_var.set(f"开始删除 {len(delete_tasks)} 个文件...")

            for action, dest_path, rel_path in delete_tasks:
                try:
                    file_size = "N/A"
                    if os.path.exists(dest_path) and os.path.isfile(dest_path):
                        file_size = self.get_file_size(dest_path)
                    ...
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    self.root.after(0, lambda: self.log_tree.insert("", 0, values=(
                        timestamp, "删除", rel_path, file_size, "成功")))
    ...
    def exit_app(self):
        ...
        try:
            # 1. 设置停止标志，防止新任务启动
            self.syncing = True  # 阻止新的同步任务
            self.stop_monitor = True  # 停止监控线程
