...
class FileSyncApp:
    def __init__(self, root):
        # 现有的初始化代码...
        self.root = root
        self.root.title("FileSync_v1.8 -- 文件同步备份工具")
        self.root.geometry("900x720")
        self.root.minsize(800, 600)

        # 添加阿里云盘相关变量
        self.aliyun_enabled = False
        self.aliyun_client = None
        self.aliyun_folder_id = None
        self.aliyun_folder_name = None
        self.aliyun_refresh_token = None
        ...
        self.stop_monitor = True  # 初始化为True，表示不监控
    ...
    def create_ui(self):
        ...
        source_entry = ttk.Entry(path_frame, textvariable=self.source_var, width=40)
        source_entry.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)

        # 添加源类型选择
        source_type_frame = ttk.Frame(path_frame)
        source_type_frame.grid(row=0, column=2, sticky=tk.EW, padx=5, pady=5)

        ttk.Button(source_type_frame, text="浏览本地...", command=self.browse_source).pack(side=tk.LEFT, padx=2)
        ttk.Button(source_type_frame, text="选择云盘...", command=self.browse_aliyundrive).pack(side=tk.LEFT, padx=2)

        # 阿里云盘设置框架
        aliyun_frame = ttk.LabelFrame(main_frame, text="阿里云盘设置", padding=10)
        aliyun_frame.pack(fill=tk.X, pady=5)

        # 阿里云盘启用复选框
        self.aliyun_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(aliyun_frame, text="使用阿里云盘作为源",
                        variable=self.aliyun_var,
                        command=self.toggle_aliyun).grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)

        # 阿里云盘状态标签
        self.aliyun_status_var = tk.StringVar(value="未登录")
        ttk.Label(aliyun_frame, textvariable=self.aliyun_status_var).grid(row=0, column=1, sticky=tk.W, padx=5, pady=5)

        # 阿里云盘登录按钮
        self.aliyun_login_button = ttk.Button(aliyun_frame, text="登录阿里云盘", command=self.login_aliyundrive)
        self.aliyun_login_button.grid(row=0, column=2, padx=5, pady=5)

        # 已选云盘文件夹显示
        ttk.Label(aliyun_frame, text="已选云盘文件夹:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        self.aliyun_folder_var = tk.StringVar(value="未选择")
        ttk.Label(aliyun_frame, textvariable=self.aliyun_folder_var).grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)

        # 云盘同步间隔设置
        ttk.Label(aliyun_frame, text="云盘同步检查间隔:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        cloud_sync_frame = ttk.Frame(aliyun_frame)
        cloud_sync_frame.grid(row=2, column=1, sticky=tk.W, padx=5, pady=5)

        self.cloud_sync_min = tk.StringVar(value="30")
        ttk.Entry(cloud_sync_frame, textvariable=self.cloud_sync_min, width=3).pack(side=tk.LEFT)
        ttk.Label(cloud_sync_frame, text="分钟").pack(side=tk.LEFT, padx=(0, 5))

        # 云盘同步按钮
        ttk.Button(aliyun_frame, text="立即同步云盘",
                   command=self.sync_aliyundrive).grid(row=2, column=2, padx=5, pady=5)
    ...
    def toggle_monitor(self):
        """切换监控状态"""
        print(f"切换监控状态: {self.monitor_var.get()}")
            ...
            try:
                if not self.syncing:
                    try:
                        ...
                        # 打印日志用于调试
                        print(f"处理队列任务: {action} - {file_path}")

                        # 检查文件/路径有效性
                        source_dir = self.source_var.get()
                        source_path = os.path.join(source_dir, file_path)

                        # 对于add和update操作，检查源文件是否存在
                        if action in ('add', 'update') and not os.path.exists(source_path):
                            print(f"忽略任务，文件不存在: {source_path}")
                        else:
    ...
    def perform_sync(self, silent=False):
        ...
        try:
            ...
            if not silent:
                messagebox.showerror("错误", f"同步过程发生错误: {str(e)}")
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "autostart": self.autostart_var.get()  # 删除sync_mode设置项
        }
    ...
    def setup_tray(self):
        ...
        # 创建系统托盘图标
        self.icon = pystray.Icon("filesync", image, "FileSync1.8", menu)
    ...
    def toggle_aliyun(self):
        """切换阿里云盘使用状态"""
        if self.aliyun_var.get():
            if not self.aliyun_client:
                # 尝试登录
                self.login_aliyundrive()
            else:
                self.aliyun_enabled = True
                self.aliyun_status_var.set(f"已登录: {self.aliyun_folder_name or '未选择文件夹'}")
        else:
            self.aliyun_enabled = False
            self.aliyun_status_var.set("未启用")

    def load_aliyun_token(self):
        """加载已保存的阿里云盘 token"""
        try:
            if os.path.exists("aliyun_token.json"):
                with open("aliyun_token.json", "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self.aliyun_refresh_token = data.get("refresh_token")
                    self.aliyun_folder_id = data.get("folder_id")
                    self.aliyun_folder_name = data.get("folder_name")

                    if self.aliyun_folder_name:
                        self.aliyun_folder_var.set(self.aliyun_folder_name)

                    # 如果有 refresh_token，尝试自动登录
                    if self.aliyun_refresh_token:
                        self.auto_login_aliyundrive()
        except Exception as e:
            print(f"加载阿里云盘 token 失败: {e}")

    def save_aliyun_token(self):
        """保存阿里云盘 token"""
        try:
            # 获取最新的 refresh_token
            if self.aliyun_client:
                self.aliyun_refresh_token = self.aliyun_client.refresh_token

            data = {
                "refresh_token": self.aliyun_refresh_token,
                "folder_id": self.aliyun_folder_id,
                "folder_name": self.aliyun_folder_name
            }

            with open("aliyun_token.json", "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"保存阿里云盘 token 失败: {e}")

    def auto_login_aliyundrive(self):
        """使用已保存的 token 自动登录阿里云盘"""
        try:
            def login_task():
                try:
                    self.aliyun_status_var.set("正在登录...")
                    self.aliyun_login_button.config(state=tk.DISABLED)

                    # 使用 refresh_token 登录
                    self.aliyun_client = aligo.Aligo(refresh_token=self.aliyun_refresh_token)

                    # 登录成功
                    self.aliyun_enabled = True
                    self.root.after(0, lambda: self.aliyun_status_var.set("已登录"))
                    self.root.after(0, lambda: self.aliyun_login_button.config(text="重新登录", state=tk.NORMAL))

                    # 保存新的 token
                    self.save_aliyun_token()
                except Exception as e:
                    self.root.after(0, lambda: self.aliyun_status_var.set(f"登录失败: {str(e)}"))
                    self.root.after(0, lambda: self.aliyun_login_button.config(state=tk.NORMAL))
                    self.aliyun_enabled = False

            # 在后台线程中执行登录
            threading.Thread(target=login_task, daemon=True).start()
        except Exception as e:
            print(f"自动登录阿里云盘失败: {e}")

    def login_aliyundrive(self):
        """登录阿里云盘"""
        try:
            def login_task():
                try:
                    ...
                    # 使用 aligo 登录（会弹出二维码）
                    self.aliyun_client = aligo.Aligo()
                    self.aliyun_refresh_token = self.aliyun_client.refresh_token

                    # 登录成功后更新 UI
                    self.root.after(0, lambda: self.aliyun_status_var.set(
                        f"已登录: {self.aliyun_folder_name or '未选择文件夹'}"))
                    self.root.after(0, lambda: self.aliyun_login_button.config(state=tk.NORMAL))
                    self.root.after(0, lambda: self.aliyun_var.set(True))
                    self.aliyun_enabled = True
                    ...
                    print(f"登录阿里云盘失败: {e}")
                    self.root.after(0, lambda: self.aliyun_status_var.set("登录失败"))
                    self.root.after(0, lambda: self.aliyun_login_button.config(state=tk.NORMAL))
                    self.root.after(0, lambda: messagebox.showerror("错误", f"登录阿里云盘失败: {str(e)}"))

            # 在后台线程中执行登录
            threading.Thread(target=login_task, daemon=True).start()
        except Exception as e:
            messagebox.showerror("错误", f"登录阿里云盘失败: {str(e)}")
            self.aliyun_login_button.config(state=tk.NORMAL)

    def browse_aliyundrive(self):
        """浏览阿里云盘选择文件夹"""
        if not self.aliyun_client:
            messagebox.showinfo("提示", "请先登录阿里云盘")
            self.login_aliyundrive()
            return

        # 创建文件夹选择窗口
        folder_win = tk.Toplevel(self.root)
        folder_win.title("选择阿里云盘文件夹")
        folder_win.geometry("600x500")
        folder_win.minsize(500, 400)

        # 文件夹树
        frame = ttk.Frame(folder_win, padding=10)
        frame.pack(fill=tk.BOTH, expand=True)

        ttk.Label(frame, text="请选择云盘文件夹:").pack(anchor=tk.W, pady=(0, 5))

        # 创建带滚动条的文件树
        tree_frame = ttk.Frame(frame)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        folder_tree = ttk.Treeview(tree_frame, columns=("name", "id"), show="tree headings")
        folder_tree.heading("#0", text="文件夹")
        folder_tree.heading("name", text="名称")
        folder_tree.heading("id", text="ID")

        folder_tree.column("#0", width=50)
        folder_tree.column("name", width=300)
        folder_tree.column("id", width=200)

        scrollbar_y = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL, command=folder_tree.yview)
        folder_tree.configure(yscrollcommand=scrollbar_y.set)

        folder_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar_y.pack(side=tk.RIGHT, fill=tk.Y)

        # 加载中提示
        loading_label = ttk.Label(frame, text="正在加载云盘文件夹，请稍候...")
        ...
        select_button = ttk.Button(button_frame, text="选择此文件夹",
                                  command=lambda: self.select_aliyun_folder(folder_tree, folder_win))
        select_button.pack(side=tk.RIGHT, padx=5)

        refresh_button = ttk.Button(button_frame, text="刷新",
                                   command=lambda: self.load_aliyun_folders(folder_tree, loading_label))
        refresh_button.pack(side=tk.RIGHT, padx=5)

        # 加载文件夹
        def load_folders():
            self.load_aliyun_folders(folder_tree, loading_label)

        threading.Thread(target=load_folders, daemon=True).start()

        # 双击选择文件夹
        folder_tree.bind("<Double-1>", lambda e: self.select_aliyun_folder(folder_tree, folder_win))

    def load_aliyun_folders(self, tree, loading_label):
        """加载阿里云盘文件夹"""
        try:
            # 清空当前树
            ...
            loading_label.config(text="正在加载云盘文件夹，请稍候...")

            # 获取根目录
            root_folder = self.aliyun_client.get_root_folder()
            root_id = tree.insert("", "end", text="", values=(root_folder.name, root_folder.file_id))

            # 递归加载文件夹
            self._load_aliyun_subfolders(tree, root_id, root_folder.file_id)

            loading_label.config(text="云盘文件夹加载完成")
        except Exception as e:
            loading_label.config(text=f"加载失败: {str(e)}")
            print(f"加载阿里云盘文件夹失败: {e}")

    def _load_aliyun_subfolders(self, tree, parent_node, folder_id):
        """递归加载阿里云盘子文件夹"""
        try:
            # 获取文件夹内容
            items = self.aliyun_client.get_file_list(folder_id)
            ...
            for folder in folders:
                node_id = tree.insert(parent_node, "end", text="", values=(folder.name, folder.file_id))
                # 为每个文件夹添加一个临时子节点，以便显示展开图标
                tree.insert(node_id, "end", text="加载中...")

            # 绑定展开事件
            def on_open(event):
                item = tree.focus()
                children = tree.get_children(item)
                if len(children) == 1 and tree.item(children[0], "text") == "加载中...":
                    # 删除临时节点
                    tree.delete(children[0])
                    # 获取文件夹ID
                    _, folder_id = tree.item(item, "values")
                    # 加载子文件夹
                    self._load_aliyun_subfolders(tree, item, folder_id)

            tree.bind("<<TreeviewOpen>>", on_open)
        except Exception as e:
            print(f"加载阿里云盘子文件夹失败: {e}")

    def select_aliyun_folder(self, tree, window):
        """选择阿里云盘文件夹"""
        ...
        item_id = selected[0]
        folder_name, folder_id = tree.item(item_id, "values")

        if not folder_id:
            messagebox.showinfo("提示", "请选择有效的文件夹")
            return

        # 保存选择的文件夹
        self.aliyun_folder_id = folder_id
        self.aliyun_folder_name = folder_name
        self.aliyun_folder_var.set(folder_name)

        # 更新源文件夹路径为阿里云盘路径标识
        self.source_var.set(f"阿里云盘:{folder_name}")
        ...
        # 自动启用阿里云盘
        self.aliyun_var.set(True)
        self.aliyun_enabled = True

    def sync_aliyundrive(self):
        """立即同步阿里云盘文件"""
        if not self.aliyun_client or not self.aliyun_folder_id:
            messagebox.showinfo("提示", "请先登录阿里云盘并选择文件夹")
            return
        ...
        if messagebox.askyesno("确认", f"确定要从阿里云盘「{self.aliyun_folder_name}」同步文件到本地吗？"):
            self.syncing = True
            self.sync_button.config(state=tk.DISABLED)
            self.status_var.set("正在同步云盘文件...")

            # 在新线程中执行同步
            sync_thread = threading.Thread(target=self.perform_aliyun_sync)
            sync_thread.daemon = True
            sync_thread.start()

    def perform_aliyun_sync(self):
        """执行阿里云盘文件同步"""
        try:
            start_time = time.time()
            dest_dir = self.dest_var.get()
            ...
            # 获取云盘文件列表（递归获取所有文件）
            self.status_var.set("正在获取云盘文件列表...")
            cloud_files = self.get_aliyun_files(self.aliyun_folder_id)

            # 同步文件
            self.status_var.set(f"找到 {len(cloud_files)} 个文件，开始同步...")

            for cloud_file in cloud_files:
                if self.stop_sync:
                    break

                rel_path = cloud_file["rel_path"]
                ...
                if need_update:
                    try:
                        ...
                        # 下载文件
                        self.aliyun_client.download_file(file_id=file_id, local_folder=local_dir)
                        ...
                        # 记录到日志
                        action = "新增" if not os.path.exists(local_path) else "更新"
                        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        file_size_str = self.get_file_size(local_path)

                        self.log_tree.insert("", 0, values=(
                            timestamp, action, rel_path, file_size_str, "成功"
                        ))
                    except Exception as e:
                        # 记录错误
                        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        self.log_tree.insert("", 0, values=(
                            timestamp, "下载", rel_path, "N/A", f"失败: {str(e)}"
                        ))
            ...
            # 更新状态
            status_text = f"云盘同步完成: {files_processed} 个文件，共 {self.format_size(total_bytes)}，耗时 {duration} 秒"
            self.status_var.set(status_text)
            ...
            messagebox.showerror("错误", f"云盘同步失败: {str(e)}")
            self.status_var.set(f"云盘同步失败: {str(e)}")
        finally:
            self.syncing = False
            self.sync_button.config(state=tk.NORMAL)

    def get_aliyun_files(self, folder_id, parent_path=""):
        """递归获取阿里云盘文件列表"""
        result = []
        try:
            # 获取文件夹内容
            ...
            for item in items:
                if item.type == "folder":
                    # 是文件夹，递归获取子文件
                    new_parent = f"{parent_path}/{item.name}" if parent_path else item.name
                    sub_files = self.get_aliyun_files(item.file_id, new_parent)
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "aliyun_enabled": self.aliyun_var.get(),
            "cloud_sync_min": self.cloud_sync_min.get()
        }
    ...
    def load_settings(self):
        ...
        try:
            ...
            # 加载阿里云盘设置
            if "aliyun_enabled" in settings:
                self.aliyun_var.set(settings["aliyun_enabled"])

            if "cloud_sync_min" in settings:
                self.cloud_sync_min.set(settings["cloud_sync_min"])
    ...
    def check_cloud_sync(self):
        ...
        try:
            # 检查是否启用了阿里云盘
            if self.aliyun_enabled and self.aliyun_client and self.aliyun_folder_id:
                current_time = time.time()
                sync_interval = int(self.cloud_sync_min.get()) * 60  # 转换为秒

                # 检查是否到达同步时间
                if current_time - self.last_cloud_sync_time > sync_interval:
                    # 执行静默同步
                    if not self.syncing:
                        print("开始自动云盘同步...")
                        self.last_cloud_sync_time = current_time
                        # 在新线程中执行同步
                        threading.Thread(target=lambda: self.perform_aliyun_sync_silent(),
                                        daemon=True).start()
        except Exception as e:
            print(f"检查云盘同步出错: {e}")

        # 每分钟检查一次
        self.root.after(60000, self.check_cloud_sync)

    def perform_aliyun_sync_silent(self):
        """静默执行阿里云盘同步（不显示消息框）"""
        if not self.aliyun_client or not self.aliyun_folder_id:
            return
        ...
        self.syncing = True
        self.sync_button.config(state=tk.DISABLED)
        self.status_var.set("正在自动同步云盘文件...")

        # 执行同步，与普通同步相同，但不显示完成消息
        try:
            start_time = time.time()
            dest_dir = self.dest_var.get()
            ...
            self.status_var.set(f"云盘自动同步完成: {datetime.now().strftime('%H:%M:%S')}")
        except Exception as e:
            print(f"自动同步云盘失败: {e}")
            self.status_var.set(f"云盘自动同步失败: {str(e)}")
        finally:
            self.syncing = False
            self.sync_button.config(state=tk.NORMAL)
