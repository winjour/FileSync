...
class FileSyncApp:
    def __init__(self, root):
        ...
        # 添加线程池配置
        self.max_workers = 4  # 可根据CPU核心数调整
        self.executor = ThreadPoolExecutor(max_workers=self.max_workers)
        self.hash_cache = {}  # 哈希缓存，减少重复计算
    ...
    def get_folder_state(self, folder_path):
        """获取文件夹中所有文件的状态（路径和哈希值），使用线程池并行计算文件哈希，提高扫描速度"""
        states = {}
        file_paths = []

        # 首先收集所有文件路径，减少I/O阻塞时间
        ...
        # 使用线程池并行计算哈希值
        results = self.executor.map(self._compute_file_hash_with_cache, file_paths)
    ...
    def _compute_file_hash_with_cache(self, file_info):
        """带缓存的文件哈希计算，单独作为函数方便线程池调用"""
        file_path, rel_path = file_info

        try:
            # 先检查文件大小和修改时间（仅用于缓存失效判断，不用于文件变化判断）
            stat = os.stat(file_path)
            file_size = stat.st_size
            mtime = stat.st_mtime
            cache_key = (file_path, file_size, mtime)

            # 检查缓存
            with self.hash_cache_lock:
                if cache_key in self.hash_cache:
                    return (rel_path, self.hash_cache[cache_key])
            ...
            # 更新缓存（限制缓存大小，防止内存溢出）
            with self.hash_cache_lock:
                if len(self.hash_cache) > 10000:  # 缓存最大10000个文件
    ...
    def get_file_hash(self, file_path):
        """优化版哈希计算：更大的缓冲区，更快的哈希算法"""
        # 对于大文件使用SHA-1替代MD5，计算速度更快
        # 两种算法都是基于内容的，满足需求
        hasher = hashlib.sha1()
        block_size = 1024 * 1024  # 1MB缓冲区，比原来的64KB大
    ...
    def perform_contribute_sync(self, source_dir, dest_dir, silent=False):
        ...
        for rel_path, src_hash in src_states.items():
            if rel_path not in dest_states:
                to_add.append((rel_path, src_hash))
            elif dest_states[rel_path] != src_hash:
                to_update.append((rel_path, src_hash))

        # 批量处理新增文件
        for rel_path, src_hash in to_add:
            self._process_add_file(rel_path, source_dir, dest_dir, stats)

        # 批量处理更新文件
        for rel_path, src_hash in to_update:
            self._process_update_file(rel_path, source_dir, dest_dir, stats)
    ...
    def _process_add_file(self, rel_path, source_dir, dest_dir, stats):
        """提取新增文件处理逻辑，便于维护和可能的并行处理"""
        try:
            source_path = os.path.join(source_dir, rel_path)
            dest_path = os.path.join(dest_dir, rel_path)
            ...
            stats["failed"] += 1
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.log_tree.insert("", 0, values=(timestamp, "新增", rel_path, "未知", f"失败: {str(e)}"))

    def _process_update_file(self, rel_path, source_dir, dest_dir, stats):
        """提取更新文件处理逻辑"""
        try:
            source_path = os.path.join(source_dir, rel_path)
            dest_path = os.path.join(dest_dir, rel_path)
            ...
            stats["failed"] += 1
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.log_tree.insert("", 0, values=(timestamp, "更新", rel_path, "未知", f"失败: {str(e)}"))


    def perform_oneway_sync(self, source_dir, dest_dir, silent=False):
        """优化版单向同步：批量处理，减少重复I/O操作"""
        # 并行获取源目录和目标目录的文件状态
        src_future = self.executor.submit(self.get_folder_state, source_dir)
        dest_future = self.executor.submit(self.get_folder_state, dest_dir)

        src_states = src_future.result()
        dest_states = dest_future.result()

        stats = {
            "added": 0, "updated": 0, "removed": 0,
            "success": 0, "failed": 0, "total_bytes": 0
        }
        ...
        if not silent:
            status_text = (f"同步完成 - 新增: {stats['added']}, "
                           f"更新: {stats['updated']}, "
                           f"删除: {stats['removed']}, "
                           f"共处理: {self.format_size(stats['total_bytes'])}")
            self.status_var.set(status_text)

        return stats

    def _batch_process_operations(self, source_dir, dest_dir, to_add, to_update, to_remove, stats):
        """批量处理所有同步操作，可进一步并行化处理"""
        # 先处理删除操作（避免先添加再删除的冲突）
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 并行处理删除操作
            delete_futures = [
                executor.submit(self._process_remove_file, rel_path, dest_dir, stats)
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 并行处理新增文件
            add_futures = [
                executor.submit(self._process_add_file, rel_path, source_dir, dest_dir, stats)
                for rel_path, _ in to_add
            ]

            # 并行处理更新文件
            update_futures = [
                executor.submit(self._process_update_file, rel_path, source_dir, dest_dir, stats)
                for rel_path, _ in to_update
            ]

            # 等待所有操作完成
            for future in add_futures + update_futures:
                future.result()

    def _process_remove_file(self, rel_path, dest_dir, stats):
        """提取删除文件处理逻辑，支持并行执行"""
        try:
            dest_path = os.path.join(dest_dir, rel_path)
            ...
            if os.path.isfile(dest_path):
                ...
                # 尝试删除空目录
                self._remove_empty_dirs(os.path.dirname(dest_path))
        except Exception as e:
            stats["failed"] += 1
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
